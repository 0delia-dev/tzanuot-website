...
let currentQuotes = [
    ...
    },
    {
        ...
        source: "הרצאות על צניעות",
        comments: []
    }
];

// Form Timing and Security
let formStartTime = null;
const minFormTime = 3000; // 3 seconds minimum
...
function toggleTheme() {
    ...
    // Animate "ויהי אור" when switching from dark to light
    if (currentTheme === 'dark' && newTheme === 'light') {
        animateLightTransition();
    }
}

function animateLightTransition() {
    const lightElement = document.getElementById('let-there-be-light');
    if (lightElement) {
        lightElement.style.display = 'block';
        lightElement.classList.remove('animate');
        
        // Force reflow
        void lightElement.offsetWidth;
        
        lightElement.classList.add('animate');
...
function validateFormSecurity(formType = 'item') {
    ...
    const websiteField = document.getElementById(formType === 'item' ? 'website' : 'question-website');
    const emailField = document.getElementById('email');
    
    if ((websiteField && websiteField.value) || (emailField && emailField.value)) {
    ...
    if (!validateCaptcha(questionId, answerId)) {
        showNotification('תשובה שגויה לבעיה המתמטית. אנא נסי שוב.');
        setupCaptcha();
        return false;
    }
...
function renderItems(itemsToRender = currentItems) {
    ...
    if (itemsToRender.length === 0) {
        grid.innerHTML = '<div class="no-results">לא נמצאו פריטים מתאימים. הוסיפי את הבגד הראשון!</div>';
        return;
    }
...
function handleAddItem(e) {
    ...
    // Basic validation
    if (!formData.title || !formData.url || !formData.sleeve_length || 
        !formData.skirt_length || !formData.collar_type) {
        showNotification('אנא מלאי את כל השדות הנדרשים');
        return;
    }
...
function submitQuestion() {
    ...
    // Security validation
    if (!validateFormSecurity('question')) {
        return;
    }
